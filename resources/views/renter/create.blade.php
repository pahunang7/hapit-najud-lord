        ...
        <form id="registerClientForm">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="preferred_location" placeholder="e.g. Glasgow">
                </div>

                <div class="form-group">
                    <label>Max Rent</label>
                    <input type="number" id="max_rent" min="0" placeholder="e.g. 1200">
                </div>
            ...
            <div class="form-actions">
                ...
                <button type="submit" id="submitBtn" class="submit-btn">Register Client</button>
            </div>
        </form>
    </div>
...
document.getElementById('branch_no').addEventListener('change', async function () {
    ...
    try {
        const response = await fetch(`/api/staff?branch_no=${branchNo}`);
        ...
        if (staff.length > 0) {
            staff.forEach(member => {
                ...
                // controller returns staff_name, not full_name
                const staffName  = member.staff_name ?? (member.first_name + ' ' + member.last_name);
                staffSelect.innerHTML += `<option value="${member.staff_no}">${staffNo} — ${staffName}</option>`;
            });
            staffSelect.disabled = false;
        } else {
            staffSelect.innerHTML = '<option value="" disabled>No staff in this branch</option>';
        }
